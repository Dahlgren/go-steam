...
func NewAuth(bot *Depot, details *LogOnDetails, sentryPath string) *Auth {
	return &Auth{
		bot:        bot,
		details:    details,
...
func NewServerList(bot *Depot, listPath string) *ServerList {
	return &ServerList{
		bot,
		listPath,
	}
...
func (s *ServerList) HandleEvent(event interface{}) {
	switch e := event.(type) {
	case *steam.LoggedOnEvent:
		servers, err := GetContentServers(int64(e.CellId))
		if err != nil {
			panic(err)
		}
...
func NewDebug(bot *Depot, base string) (*Debug, error) {
